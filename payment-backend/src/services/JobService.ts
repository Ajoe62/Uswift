...
class JobService {
  ...
  async createJob(params: CreateJobParams): Promise<Job> {
    try {
      ...
      logger.info('Job created', { jobId: id, userId: params.userId });

      return job;
    } catch (error) {
      logger.error('Failed to create job', { error, params });
      throw error;
    }
  }
   ...
   * Update job
   */
  async updateJob(jobId: string, params: UpdateJobParams): Promise<Job | null> {
    try {
      ...
      logger.info('Job updated', { jobId, updates: Object.keys(params) });

      return result.rows[0] || null;
    } catch (error) {
      logger.error('Failed to update job', { error, jobId, params });
      throw error;
    }
  }
   ...
   */
  async generateInstructionToken(
    jobId: string,
    instructions: any
  ): Promise<string> {
    try {
      ...
      logger.info('Instruction token generated', { jobId });

      return token;
    } catch (error) {
      logger.error('Failed to generate instruction token', { error, jobId });
      throw error;
    }
  }
  ...
  verifyInstructionToken(token: string): {
    ...
    try {
      ...
      logger.warn('Invalid instruction token', { error });
      return { valid: false };
    }
  }
   ...
   * Add audit log entry
   */
  async addAuditLog(log: JobAuditLog): Promise<void> {
    try {
      await db.query(
        `
      ...
      logger.error('Failed to add audit log', { error, log });
   ...
   * Get job audit logs
   */
  async getAuditLogs(jobId: string): Promise<any[]> {
    try {
      ...
      logger.error('Failed to get audit logs', { error, jobId });
      throw error;
    }
  }
  ...
  async recordCaptchaEvent(
    ...
    try {
      ...
      logger.info('CAPTCHA event recorded', { jobId, captchaType });
    } catch (error) {
      logger.error('Failed to record CAPTCHA event', { error, jobId });
      throw error;
    }
  }
  ...
  async recordConsentRequest(
    ...
    try {
      ...
      logger.info('Consent request recorded', { jobId, consentType });
    } catch (error) {
      logger.error('Failed to record consent request', { error, jobId });
      throw error;
    }
  }
