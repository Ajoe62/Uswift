...
export default function JobTracker() {
  ...
  const addApplication = async () => {
    ...
    if (isAuthenticated && user) {
      ...
      if (id) {
        const appWithId = { ...newApplication, id: typeof id === 'string' ? id : String(id) };
  ...
  return (
    <div style={{ background: '#FFFFFF', borderRadius: '1.5rem', padding: '2rem', maxWidth: '100%', overflow: 'hidden' }}>
      <div className="uswift-gradient" style={{ height: 8, borderRadius: 8, marginBottom: 24 }} />
      
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h2 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#111827' }}>Job Tracker</h2>
        {isAuthenticated && (
          <span style={{ color: '#10B981', fontSize: '0.8rem', fontWeight: 600 }}>☁️ Cloud Sync</span>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 12, marginBottom: 20 }}>
        <div style={{ background: '#F3F4F6', padding: 12, borderRadius: 8, textAlign: 'center' }}>
          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#3B82F6' }}>{getStatusCount('applied')}</div>
          <div style={{ fontSize: '0.8rem', color: '#6B7280' }}>Applied</div>
        </div>
        <div style={{ background: '#F3F4F6', padding: 12, borderRadius: 8, textAlign: 'center' }}>
          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#F59E0B' }}>{getStatusCount('interviewing')}</div>
          <div style={{ fontSize: '0.8rem', color: '#6B7280' }}>Interviewing</div>
        </div>
        <div style={{ background: '#F3F4F6', padding: 12, borderRadius: 8, textAlign: 'center' }}>
          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#10B981' }}>{getStatusCount('offer')}</div>
          <div style={{ fontSize: '0.8rem', color: '#6B7280' }}>Offers</div>
        </div>
        <div style={{ background: '#F3F4F6', padding: 12, borderRadius: 8, textAlign: 'center' }}>
          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#EF4444' }}>{getStatusCount('rejected')}</div>
      ...
      {showAddForm && (
        <div style={{ background: '#F9FAFB', padding: 16, borderRadius: 8, marginBottom: 16 }}>
          <h3 style={{ marginBottom: 12, fontSize: '1rem', fontWeight: 600 }}>Add New Application</h3>
          ...
          filteredApplications.map(app => (
            <div key={app.id} style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: 4, color: '#111827' }}>
                    {app.position}
                  </h3>
                  <p style={{ color: '#6B7280', fontSize: '0.9rem', marginBottom: 4 }}>
                    {app.company} • Applied {new Date(app.dateApplied).toLocaleDateString()}
                  </p>
                  {app.jobUrl && (
                    <a 
                      href={app.jobUrl} 
                      ...
                      style={{ color: '#6D28D9', fontSize: '0.85rem', textDecoration: 'none' }}
                    >
                      View Job Posting →
                    </a>
                  )}
                  {app.notes && (
                    <p style={{ color: '#4B5563', fontSize: '0.85rem', marginTop: 8 }}>
                      {app.notes}
                    </p>
                  )}
                </div>
