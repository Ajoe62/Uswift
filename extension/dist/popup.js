...
const JobTracker = () => {
  ...
  const [loading, setLoading] = reactExports.useState(false);
  const [newApplication, setNewApplication] = reactExports.useState({
    company: "",
    position: "",
    status: "applied",
  ...
  const saveToSupabase = async (application) => {
    try {
      if (!user?.id)
        return false;
      const appData = {
...
if (typeof window !== 'undefined' && window.SUPABASE_CONFIG) {
  supabase$1 = new SupabaseClient(window.SUPABASE_CONFIG);
  // Load existing session on initialization
  supabase$1.loadSession();
}
